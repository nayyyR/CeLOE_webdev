<x-layouts.app :title="$ticket->ticket_number . ' - Celoe'">
    <div class="mb-8 flex items-center justify-between">
        ...
        <a href="{{ route('user.tickets.index') }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">Back to List</a>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if ($ticket->status !== 'closed')
                <div class="rounded-xl bg-white shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <form method="POST" action="{{ route('user.tickets.reply', $ticket) }}" enctype="multipart/form-data" class="p-6">
                        @csrf
                        <div>
                    ...
                    @if ($ticket->status === 'resolved')
                        <div class="space-y-2">
                            ...
                            <form method="POST" action="{{ route('user.tickets.status', $ticket) }}">
                                ...
                                <button type="submit" class="w-full inline-flex items-center justify-center rounded-lg border border-red-300 bg-white px-4 py-2 text-sm font-medium text-red-700 shadow-sm hover:bg-red-50 transition-colors">
                                    Reject Resolution
                                </button>
                            </form>
                        </div>
