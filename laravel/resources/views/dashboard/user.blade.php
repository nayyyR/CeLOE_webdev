<x-layouts.app :title="'Dashboard - Celoe'">
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 overflow-hidden">
        <div class="border-b border-gray-200 bg-gray-50 px-6 py-4 flex items-center justify-between">
            ...
            <a href="{{ route('user.tickets.index') }}" class="text-xs font-medium text-indigo-600 hover:text-indigo-500">View all</a>
        </div>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse ($recentTickets as $ticket)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-xs text-gray-500">{{ $ticket->created_at->format('d M Y') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">No tickets yet. <a href="{{ route('user.tickets.create') }}" class="text-indigo-600 hover:underline">Create your first ticket</a></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
